...
imageindex_init=1;
filenameraw=folderraw+'/b_mehta_'+basename+str(imageindex_init)+'_0000.raw'
...
for i in range(1, imageindex_max):
    ...
    a=f"{foils:04d}"
    seconds=dataCSV[1,4]
    fabs.append(1/seconds/np.exp(-int(a[0])*abs[0]-int(a[1])*abs[1]-int(a[2])*abs[2]-int(a[3])*abs[3]))
...
#    {"#": imageindex}
#    filenameraw=folderraw+'/b_mehta_'+basename+'_'+strimageindex+'.raw';
    filenameraw=folderraw+'/b_mehta_'+basename+str(int(imageindex))+'_0000.raw'
    ...
    Image.normlz=img*fabs[int(imageindex-1)];
    #normlz=img;
...
#    filenamePDI=folderpdi+'/b_mehta_'+basename+'_'+strimageindex+'.raw.pdi';
    filenamePDI=folderraw+'/b_mehta_'+basename+str(int(imageindex))+'_0000.raw.PDI'
...
#        Int1=self.imgplot.get_array();
        ...
        #self.ROI.center
    def storedata(self):
        imageindex=int(self.img_slider.val-1)
...
def keyboard_selector(event):
    if event.key in ['e','enter']:
        img_slider.set_val(int(img_slider.val+1))
    ...
    if event.key in ['z']:
        ...
        update(img_slider.val)
        for ii in range(0,scan.countsROIS()):
            scan.selectorlist[ii].ROI.extents=scan.ROIlist[ii].saveROI[int(img_slider.val-1),:]
    if event.key in ['a']:
        img_slider.set_val(int(img_slider.val+1))
        update(img_slider.val)
        for ii in range(0,scan.countsROIS()):
...
def buttonzoom1fun(event):
    ax1.set_xlim(244-50,244+50)
    ax1.set_ylim(122-20,122+20)
