...
#plt.suptitle(basename+', imageindex = '+str(imageindex) +', abs='+str(int(dataCSV[imageindex,13])))
...
def ROIlineupdate(ROI):
    ...
    print(ROI.saveInt.max())
    plt.setp(ROIplot0, ydata=ROI.saveInt)
    ax5.set_ylim(1,1e11)
...
class ROIselector(object):
    ...
    def updatedata(self):
        ...
        IntBKGR=np.average(Int1[ROI[3]+1,ROI[0]:ROI[1]]);
        self.IntBKGR=IntBKGR;
        self.IntSumNormSub=self.IntSumNorm-self.IntBKGR
    def storedata(self):
        print('storing data')
        imageindex=int(self.img_slider.val)
        print(imageindex)
        self.saveInt[imageindex]=self.IntSumNormSub
        self.saveROI[imageindex,:]=self.bounds
        print(self.saveInt[imageindex])
        #self.boundsSave[imageindex]=self.boundsss
#    imageindex=int(img_slider.val);
#    ROIIntline[iCTR,imageindex]=RectangleSelector.IntSumNorm;
#    plt.setp(ROIplot0, ydata=ROIIntline[0,:])


   
ROI1=ROIselector(ax1,imgplot,img_slider);
#ROI1.updatedata(imgplot)
